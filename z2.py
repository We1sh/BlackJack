...
class BJ_Game: 
    ...
    def play(self): 
        ...
        for i in self.players: 
            i.bid = games.ask_num(f"Какая ставка у игрока {i.name} : ", l=1, h=i.bank + 1) 
...
def main(): 
    ...
    names = [] 
    banks = [] 
    number = games.ask_num("Сколько всего игроков?(1 - 7): ", 1, 8) 
 
    for i in range(number): 
        name = input(f"Введите имя игрока № {i+1} : ") 
        bank = games.ask_num(f"Введите банк игрока № {i+1} : ") 
        names.append(name) 
        banks.append(bank) 
    print() 
